...
; Start digit is taken from DIP on A4..A7 (0..9) when switch turns ON.
...
; Returns with ZF=1 if switch went OFF during the delay, ZF=0 if delay elapsed.
...
;============== GET_INPUT (A4..A7 -> DIGIT 0..9) ==============
; Reads upper nibble of PORTC, shifts to low nibble, clamps to 9,
; writes DIGIT and immediately updates displays.
; Note: assumes DIP is active-high. If active-low, invert with XOR AL,0Fh before clamp.
GET_INPUT:
 PUSH AX
 PUSH CX
 PUSH DX
...
;============== SHOW_DIGIT: writes 7-seg (PORTB) and LEDs (PORTA) ==============
SHOW_DIGIT:
 PUSH AX
 PUSH BX
 PUSH DX
