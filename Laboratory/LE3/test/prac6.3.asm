; ==========================================
; OCTAL (up to 3 digits) -> Decimal, Binary (8 bits), Octal (3 digits), Hex (2 digits)
; EMU8086 .COM
; ==========================================
...
; ---------- DATA ----------
prompt      db 'Enter octal (up to 3 digits): $'
nl          db 0Dh,0Ah,'$'
...
start:
    ...
    ; -------- read & parse OCTAL (up to 3 digits) --------
    xor bl, bl                 ; BL = accumulator for final value (0..255)
    xor cx, cx                 ; CX = digit count (0..3)
read_loop:
    mov ah, 1
    int 21h
    cmp al, 0Dh                ; Enter?
    ...
    ; ignore spaces/underscores; accept only '0'..'7'
    cmp al, ' '
    je  read_loop
    cmp al, '_'
    je  read_loop
    ...
    ; compute new_value = bl*8 + (al-'0'); ensure <= 255
    mov dl, al
    sub dl, '0'
    xor ah, ah
    mov al, bl
    ...
    shl ax, 1
    shl ax, 1                  ; AX = BL * 8
    add al, dl                 ; AX = BL*8 + digit
    adc ah, 0
    cmp ax, 255
    ja  read_loop              ; ignore digit if it would overflow >255 (e.g., 400o)
    mov bl, al                 ; commit
    inc cx
    jmp read_loop
