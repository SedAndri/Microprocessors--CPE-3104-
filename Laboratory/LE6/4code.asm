...
WaitWake:
    ...
    CALL LCD_CLEAR
    MOV  LCD_ON_FLAG, 1
    CALL WAIT_KEY_RELEASE       ; consume the held '*'
    JMP  ShowMenu
CheckHashOff_WhileOff:
    CMP  AL, '#'
    JNE  ReleaseAndWaitWake
    MOV  CX, LP_HASH_5S         ; ignore while already OFF, just consume press
    CALL IS_LONG_PRESS
ReleaseAndWaitWake:
...
MenuChoice:
    ; Poll keys so we can detect '#' long-press for power-off
MC_Poll:
    CALL WAIT_FOR_PRESS_ASCII   ; AL = ascii, no release wait
    CMP  AL, '#'
    JNE  MC_NotHash
    ; Check 5s long-press to power OFF
    MOV  CX, LP_HASH_5S
    CALL IS_LONG_PRESS          ; AL=1 if sustained
    CMP  AL, 1
    JNE  MC_HashShort
    ; Power OFF and go to Standby
    CALL LCD_DISPLAY_OFF
    MOV  LCD_ON_FLAG, 0
    CALL WAIT_KEY_RELEASE
    JMP  Standby
MC_HashShort:
    ; short '#' does nothing here
    CALL WAIT_KEY_RELEASE
    JMP  MC_Poll
...
PFZO_L:
    ...
    ; Handle '#' long-press (5s) -> power OFF
    CMP  AL, '#'
    JNE  PFZO_CHECK_ZERO
    MOV  CX, LP_HASH_5S
    MOV  AL, '#'
...
DELAY_1MS:
    ; Calibrate to your board (~0.625x previous 48-iter loop).
    ; If still off by a few %, tweak to 29..31.
    PUSH CX
    MOV  CX, 1EH          ; 30 iterations ≈ 1 ms on your hardware
DLY1:
    NOP
    LOOP DLY1
    POP  CX
...
DELAY_250MS PROC
    PUSH CX
    MOV  CX, 250          ; ≈250 ms (uses 1 ms tick)
D250_L:
    CALL DELAY_1MS
    LOOP D250_L
    POP  CX
...
DELAY_750MS PROC
    PUSH CX
    MOV  CX, 750          ; ≈750 ms (uses 1 ms tick)
D750_L:
    CALL DELAY_1MS
    LOOP D750_L
    POP  CX
